<?php
	...
	$review_sites = [
		...
		[
			...
			"stars_regex"	=>		"\<span.*?class\=\"rtng\".*?\>.*?([0-9.]{1,4}).*?\<", // RegEx pattern to extract the stars score
			"count_regex"	=>		"\<span\>([0-9,]+).*?reviews.*?\<", // RegEx pattern to extract the reviews counts
		],
		...
		[
			...
			"max_score"		=>		5, // The maximum score/star rating on this review site (normally 5)
			"stars_regex"	=>		"\<script.*?ld\+json.*?\>.*?aggregateRating.*?ratingValue[\"': ]+([0-9.]{1,4}).*?\<", // RegEx pattern to extract the stars score
			"count_regex"	=>		"\<script.*?ld\+json.*?\>.*?aggregateRating.*?ratingCount[\"': ]+([0-9,]{1,8}).*?\<", // RegEx pattern to extract the reviews counts
		],
		...
		[
			...
			"max_score"		=>		5, // The maximum score/star rating on this review site (normally 5)
			"stars_regex"	=>		"itemprop.*?=[\"' ]*?ratingValue[\"' ]*?content[\"' ]*?\=[\"' ]*?([0-9.]{1,4})[\"' ]*?", // RegEx pattern to extract the stars score
			"count_regex"	=>		"itemprop.*?=[\"' ]*?ratingCount[\"' ]*?content[\"' ]*?\=[\"' ]*?([0-9,]{1,8})[\"' ]*?", // RegEx pattern to extract the reviews counts
		],
		...
		[
			...
			"stars_regex"	=>		"ratingValue[\"' ]*?\>.*?([0-9.]{1,4}).*?\<", // RegEx pattern to extract the stars score
			"count_regex"	=>		"ratingCount[\"' ]*?\>.*?([0-9,]{1,8}).*?\<", // RegEx pattern to extract the reviews counts
		],
